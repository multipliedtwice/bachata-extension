/**
 * Webview copy of src/state/boundedAgentOutput.ts.
 *
 * The webview bundle is concatenated (tsconfig.webview.json, module: none, outFile), so it
 * cannot import the extension-host module. tests/boundedAgentOutput.test.cjs compares the two
 ...
 */
const AGENT_OUTPUT_UNITS = 256 * 1024;

const AGENT_OUTPUT_ELISION = "[earlier output not shown]\n";

const boundedAgentOutput = (text: string): string => {
