...
const roomHeaderHtml = (
  ...
  const showViewSwitch = views.direction || views.execution || state.roomView !== "chat";
  const viewSwitch = showViewSwitch
    ? `<div class="view-switch" role="group" aria-label="Run view">${views.direction || state.roomView === "direction" ? `<button data-action="room-view" data-view="direction" class="${state.roomView === "direction" ? "selected" : ""}" aria-pressed="${state.roomView === "direction" ? "true" : "false"}">Direction</button>` : ""}<button data-action="room-view" data-view="chat" class="${state.roomView === "chat" ? "selected" : ""}" aria-pressed="${state.roomView === "chat" ? "true" : "false"}">Chat</button>${views.execution || state.roomView === "execution" ? `<button data-action="room-view" data-view="execution" class="${state.roomView === "execution" ? "selected" : ""}" aria-pressed="${state.roomView === "execution" ? "true" : "false"}">Execution${blockingCount > 0 ? ` (${String(blockingCount)})` : ""}</button>` : ""}</div>`
    : "";
  return `<header class="room-header"><h1 class="sr-only">${escapeHtml(conversation.title)}</h1><div class="room-utility"><div class="room-utility-lead">${parentNavigation}${statusPill}${context}</div><div class="room-actions">${interrupt}${notificationBellHtml()}${viewSwitch}<details class="header-action-menu" ${disclosureAttributes(`header-menu:${conversation.id}`)}><summary aria-label="Run actions" title="Run actions"><i class="codicon codicon-ellipsis" aria-hidden="true"></i></summary><div>${moreActions}</div></details></div></div></header>`;
};
...
const mainRoomHtml = (): string => {
  ...
  return `<section class="room-shell">${roomHeaderHtml(panel, conversation, { direction: hasDirectionState(), execution: hasExecutionState })}${archivedBanner}${blockingBanner}<div class="room-body ${state.inspectorOpen ? "with-inspector" : ""}"><main class="conversation-column"${inspectorCoversRoom() ? " inert" : ""}><div class="conversation-scroll ${introNeeded && state.roomView !== "execution" ? "is-empty" : ""}" id="conversation-scroll">${content}${state.roomView === "chat" ? "" : panel.transcriptError ? `<p class="error-banner">${escapeHtml(panel.transcriptError)}</p>` : ""}</div>${readOnly || state.roomView !== "chat" ? "" : composerHtml(panel, draft)}</main>${inspectorHtml(panel, readOnly)}</div></section>`;
};
