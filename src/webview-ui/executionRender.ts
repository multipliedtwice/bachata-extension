...
const recoveryModelActionHtml = (
  panel: PanelState,
  recovery: RunRecovery | undefined,
  agentId: string | undefined,
): string => recovery && agentsAssignable(panel) && agentsModelLockReason(panel) === undefined
  ? `<button data-action="recovery-change-model"${agentId ? ` data-agent="${escapeAttribute(agentId)}"` : ""}>${escapeHtml(localize("Change models"))}</button>`
  : "";
...
const resultCenterHtml = (conversationId: string, panel: PanelState): string => {
  ...
  return `<section class="result-center" data-code-scroll-surface="${escapeAttribute(`result:${conversationId}`)}">
    <header><div><span class="decision-label">${escapeHtml(result.expectations?.changedFiles === false ? localize("Review report") : localize("Run result"))}</span><h2>${escapeHtml(resultHeadlineLabel(result, panel.resumableWorkflow?.outcome))}</h2>${recovery ? `<p class="result-recovery-position">${escapeHtml(recoveryPositionText(panel, recovery))}</p>` : ""}</div><div class="compact-actions result-primary-actions">${recoveryModelActionHtml(panel, recovery, result.finalAssessment?.failure?.agentId)}${recoveryActionsHtml(panel, recovery)}${resultCopyActionHtml(conversationId, result)}${result.retainedWorktree && orchestrationRunId ? `<button data-action="orchestration-reveal" data-run-id="${escapeAttribute(orchestrationRunId)}" data-conversation="${escapeAttribute(conversationId)}">${escapeHtml(localize("Reveal worktree"))}</button>` : ""}<details class="header-action-menu wide-trigger" ${disclosureAttributes(`result-export:${conversationId}`)}><summary aria-label="${escapeAttribute(localize("Run result actions"))}" title="${escapeAttribute(localize("Run result actions"))}">${escapeHtml(localize("More"))}</summary><div>${recoverySecondaryActionsHtml(panel, recovery)}<button data-action="result-publish-findings">${escapeHtml(localize("Publish findings to Problems"))}</button><button data-action="result-source-control">${escapeHtml(localize("Open Source Control"))}</button><button data-action="run-bundle-export" data-format="bundle" data-conversation="${escapeAttribute(conversationId)}">${escapeHtml(localize("Run bundle (JSON)"))}</button><button data-action="run-bundle-export" data-format="executionEvidence" data-conversation="${escapeAttribute(conversationId)}">${escapeHtml(localize("Admitted execution evidence (JSON)"))}</button><button data-action="run-bundle-export" data-format="markdown" data-conversation="${escapeAttribute(conversationId)}">${escapeHtml(localize("Evidence report (Markdown)"))}</button><button data-action="run-bundle-export" data-format="sarif" data-conversation="${escapeAttribute(conversationId)}">${escapeHtml(localize("Evidence findings (SARIF)"))}</button></div></details></div></header>
