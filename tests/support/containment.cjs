 ...
 * refusal.
 *
 * This is the CommonJS counterpart of `browser-bridge/scripts/lib/containment.mjs`, kept in step
 * with it deliberately: the two repositories ship separately, and a containment rule that held in
...
const nonCanonicalTarget = (target) =>
 ...
 */
const escapesRoot = (root, target) => {
  const relative = path.relative(path.resolve(root), path.resolve(target));
  return relative.startsWith("..") || path.isAbsolute(relative);
};

const symlinkAncestorProblem = (root, target, lstat = lstatSync, resolveReal = realpathSync) => {
...
const containmentStatement = (problem) =>
...
module.exports = { containmentStatement, escapesRoot, nonCanonicalTarget, symlinkAncestorProblem };
