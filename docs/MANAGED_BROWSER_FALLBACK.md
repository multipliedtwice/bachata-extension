...
- read one readable text file or line range; full reads carry an issued opaque `fileVersion`, while ranged reads carry only a range-scoped reference and require an explicit `context.fileVersion` action before a mutation;
