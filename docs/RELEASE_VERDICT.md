...
Linux passed all gates. macOS failed a fixed-delay UI assertion; the harness now waits
for actual DOM state with a five-second bound and passed locally at all five widths.
Hosted rerun required. Windows remains in progress; no passing result inferred.
