...
const vsixName = `bachata-vscode-${packageJson.version}.vsix`;
const vsixPath = path.join(root, vsixName);

// Binding records an existing candidate's hash; it never builds one. An unpackaged tree is the
// ordinary state before the identity gate clears, so it is reported as one instruction rather
// than as an unhandled ENOENT, and no document is touched.
let vsixSnapshot;
try {
  vsixSnapshot = await readArtifactSnapshot(
    vsixPath,
  ...
  console.error(
    ...
    + "release:bind writes measured hashes into the release records; it does not build the\n"
    + "candidate they name. Build it with\n"
    + "  npm run package\n"
    + "and run this command again. No document was changed.",
  );
  process.exitCode = 1;
}
